...
class CSIQFolder(data.Dataset):
    ...
    def __init__(self, root, index, transform, patch_num):
        ...
        labels_type = dmos['distortion_types'].astype(np.long)
        ...
        for i, item in enumerate(index):
            ...
            for j, item in enumerate(train_sel):
                for aug in range(patch_num):
                    sample.append((os.path.join(root, 'dst_imgs_all', imgnames[item]), labels[item], labels_type[0][item]))
        self.samples = sample
    ...
    def __getitem__(self, index):
        ...
        """
        path, target_score, target_type = self.samples[index]
        sample = pil_loader(path)
        sample = self.transform(sample)

        return sample, target_score, target_type
...
class LIVEFolder(data.Dataset):
    ...
    def __init__(self, root, index, transform, patch_num):
        ...
        dmos = scipy.io.loadmat(os.path.join(root, 'dmos_realigned.mat'))
        labels_score = dmos['dmos'].astype(np.float32)
        labels_type = dmos['distortion_types'].astype(np.long)
        ...
        for i in range(0, len(index)):
            ...
            for j, item in enumerate(train_sel):
                for aug in range(patch_num):
                    sample.append((imgpath[item], labels_score[0][item], labels_type[0][item]))
        self.samples = sample
        self.transform = transform
